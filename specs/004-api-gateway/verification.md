...
- [-] VER-006: Test names clearly reference AC or user story IDs (N/A - tests use descriptive names instead)
- [-] VER-007: Test output confirms which AC is being verified (N/A - not using verbose AC tracing)
...
- [-] VER-016: Property-based tests exist for complex logic (scoring, routing, etc.) using `proptest` (N/A - API Gateway has simple pass-through logic; property tests in registry)
...
- [-] VER-023: **Property-based tests** validate scoring/routing invariants (N/A - no complex scoring in API Gateway)
...
- [-] VER-040: Routing decision completes in < 1ms (measured with benchmark or tracing) (N/A - basic backend selection, no complex routing yet)
- [x] VER-041: Total request overhead is < 5ms (measured: total_time - backend_processing_time)
- [-] VER-042: Memory baseline is < 50MB at startup (measured with profiler) (N/A - not profiled but binary is 6.3MB)
- [-] VER-043: Memory per backend is < 10KB (measured with 100+ backends registered) (N/A - measured in registry feature)
- [x] VER-044: Performance benchmarks pass (if defined in spec)
...
- [-] VER-089: Memory leaks are absent (long-running test shows stable memory usage) (N/A - not profiled in detail)

### Resource Limits (NFR-Resources)

- [-] VER-090: Memory usage at startup is < 50MB (baseline) (N/A - not profiled)
- [-] VER-091: Memory usage per backend is < 10KB (measured with 100+ backends) (N/A - registry feature responsibility)
...
- [-] VER-117: Backend selection logic is correct (N/A - basic retry, F05 Intelligent Router not yet implemented)
- [x] VER-118: Retry logic works (tries next backend on failure)
- [-] VER-119: Fallback chains are respected (if configured) (N/A - not yet implemented)
- [-] VER-120: Model aliases are resolved correctly (if configured) (N/A - not yet implemented)
...
- [x] VER-126: All CLI commands work as specified
...
- [-] VER-144: README.md is updated with new feature information (if user-facing) (N/A - README exists but API Gateway details in spec/walkthrough)
- [-] VER-145: ARCHITECTURE.md is updated (if architecture changed) (N/A - no separate ARCHITECTURE.md)
- [-] VER-146: FEATURES.md lists new feature (if applicable) (N/A - no separate FEATURES.md)
- [x] VER-147: Example config updated (if new config options added) - nexus.example.toml exists

### Spec Documentation

- [x] VER-148: Spec status updated to "✅ Implemented" in `spec.md`
- [x] VER-149: All tasks in `tasks.md` have checked acceptance criteria
- [-] VER-150: PR link is added to spec.md (if merged) (N/A - no PR link in spec yet)
...
- [-] VER-159: Release notes drafted (if applicable) (N/A - no release yet)

### Git & PR Hygiene

- [-] VER-160: Feature branch is up-to-date with main (N/A - implemented on main)
- [x] VER-161: All commits follow conventional commit format
- [-] VER-162: PR description links to spec and closes related issues (N/A - no PR yet)
- [-] VER-163: No merge conflicts exist (N/A - no PR yet)
- [-] VER-164: PR has been reviewed (if team review required) (N/A - no PR yet)
...
- [-] VER-173: **vLLM integration**: Connect to real vLLM instance → models discovered and usable (N/A - no vLLM test environment)
...
- [-] VER-181: **OpenAI Python SDK**: Requests succeed with official SDK (N/A - not tested with actual SDK)
- [-] VER-182: **Claude Code**: Nexus works as OpenAI proxy in Claude Code settings (N/A - manual testing only)
- [-] VER-183: **Continue.dev**: Nexus works in Continue.dev config (N/A - manual testing only)
- [-] VER-184: **Cursor**: Nexus works as custom OpenAI endpoint in Cursor (N/A - manual testing only)

### Backend Compatibility

- [x] VER-185: **Ollama**: All model queries and completions work correctly
- [-] VER-186: **vLLM**: All model queries and completions work correctly (N/A - no vLLM test env)
- [-] VER-187: **llama.cpp**: All model queries and completions work correctly (if supported) (N/A - no llama.cpp test env)
- [-] VER-188: **OpenAI API**: Direct proxy to OpenAI API works (if supported) (N/A - no OpenAI API key for testing)
...
- [-] VER-205: ✅ **Intelligent Routing** - Routing considers capabilities first, then load/latency (N/A - basic retry logic, F05 not yet implemented)
...
- [-] VER-209: **Reviewer sign-off** - Code review completed and approved (if applicable) (N/A - no formal review)
- [-] VER-210: **QA sign-off** - Manual testing completed (if applicable) (N/A - no QA team)
