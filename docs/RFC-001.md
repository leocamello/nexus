...
src/
├── routing/
│   ├── mod.rs               # Router: builds intent → runs pipeline → returns result
│   └── reconciler/          # Reconciler pipeline (co-located with routing)
│       ├── mod.rs           # Reconciler trait, ReconcilerPipeline runner
│       ├── intent.rs        # RoutingIntent with rejection_reasons
│       ├── decision.rs      # RoutingDecision: Route | Queue | Reject
│       ├── request_analyzer.rs # RequestAnalyzer (alias resolution + F15 foundation)
│       ├── privacy.rs       # PrivacyReconciler (F13)
│       ├── budget.rs        # BudgetReconciler (F14)
│       ├── tier.rs          # TierReconciler (F13)
│       ├── quality.rs       # QualityReconciler (F15 foundation)
│       ├── scheduler.rs     # SchedulerReconciler (scoring + scheduling)
│       └── scheduling.rs    # AgentSchedulingProfile
```

> **Implementation Note**: The RFC originally prescribed a separate `src/control/` module.
> During implementation, we co-located reconcilers under `src/routing/reconciler/` to keep
> routing logic together. AliasReconciler was folded into `RequestAnalyzer` since alias
> resolution is a pre-processing step, not a policy reconciler.
