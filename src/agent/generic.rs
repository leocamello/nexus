...
mod tests {
    ...
    async fn test_list_models_api_error() {
        ...
        let agent = test_agent(server.url(), BackendType::Exo);
    ...
    #[test]
    fn test_name_heuristics_tool_keyword() {
        let mut model = ModelCapability {
    ...
    #[test]
    fn test_fallback_profile_for_unexpected_backend_type() {
        let agent = test_agent("http://localhost:9000".to_string(), BackendType::OpenAI);
        let profile = agent.profile();
        assert_eq!(profile.backend_type, "generic"); // Fallback
    }

    #[test]
    fn test_profile_with_capability_tier() {
        let client = Arc::new(Client::new());
        let agent = GenericOpenAIAgent::new(
            "test".to_string(),
            "Test".to_string(),
            BackendType::VLLM,
    ...
    #[tokio::test]
    async fn test_list_models_invalid_json_body() {
        let mut server = Server::new_async().await;
        let mock = server
            .mock("GET", "/v1/models")
            .with_status(200)
    ...
    #[tokio::test]
    async fn test_health_check_network_failure() {
        let agent = test_agent(
            "http://invalid-host-that-does-not-exist:9999".to_string(),
            BackendType::VLLM,
        );
